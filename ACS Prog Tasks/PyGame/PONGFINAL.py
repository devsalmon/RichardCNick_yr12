...
#ENDDEF
def draw_winner(screen, x, y, winner):
    # Select the font to use, size, bold, italics
    font = pygame.font.SysFont('Calibri', 50, True, False)
    # Render the text. "True" means anti-aliased text.
    # SALMON is the colour. The variable SALMON was defined
    ...
    # Note: This line creates an image of the letters,
    # but does not put it on the screen yet.
    text = font.render(str(winner), True, SALMON)
    # Puts the image of the text on the screen at x,y
    screen.blit(text, (x, y))
#ENDDEF
...
# -- User input and controls
    ...
    if x_val <= padd_width and y_val < (y_padd + padd_length) and y_val > (y_padd - padd_length):
        ...
        #PLAYS PONG SOUND
        pygame.mixer.music.load('Pong_short.mp3')
    ...
    if scoreA == 3 or scoreB == 3:
        ...
        y_direction = 0
        if scoreA > scoreB:
            draw_winner(screen, 150, winner_y_position, "Player A has won")
        else:
            draw_winner(screen, 150, winner_y_position, "Player B has won")
        winner_y_position = winner_y_position + winner_direction
        #Controls winner text movement
        if winner_y_position == size[1] - 50:
            winner_direction = winner_direction * -1
        elif winner_y_position == 0:
            winner_direction = winner_direction * -1
        #ENDIF
